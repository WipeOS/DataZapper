...
if [ "$drive" == "" ]; then
	printf "archive_log.sh: No FAT32 formatted drive found, unable to archive datazapper log file\n"
	exit 1
else
	printf "Archiving datazapper logs to $drive\n"
fi
...
if [ $status != 0 ] && [ $status != 32 ]; then
    ...
    # Copy the datazapper log files over to the FAT32 partition
    sed -i '$ d' /datazapper_log*
    cp /datazapper_log* "$archive_drive_directory/"

    if [ $? != 0 ]; then
        printf "archive_log.sh: Unable to copy the datazapper log files to the root of $drive:/\n"
    else
        printf "archive_log.sh: Successfully copied the datazapper logs to $drive:/\n"
        ...
        if [ exit_code != 5 ]; then
                # Move the datazapper logs into the RAM disc sent directory
                mv /datazapper_log* "$sent_directory/"

                if [ $? != 0 ]; then
					printf "archive_log.sh: Unable to move the datazapper logs into the $sent_directory on the RAM disc\n"
					exit_code=6
                else
					printf "archive_log.sh: Moved the datazapper logs into the $sent_directory\n"
                fi
        fi
    fi
fi
...
if [ $exit_code != 0 ]; then
    printf "archive_log.sh: Failed to copy datazapper log files to $drive, exit code $exit_code\n"
fi
